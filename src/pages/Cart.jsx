...
export default function Cart() {
    ...
    const onClearCart = () => {
        let confirm = () => window.confirm("Вы уверены, что хотите удалить все пиццы?")
        if (confirm()) {
            dispatch(clearCart())
        }
    ...
    const { totalPrice, totalPizzasCount, items } = useSelector(({ cart }) => cart)
    ...
    return (
        ...
        <div className="content">
                            ...
                            <div className="content__items">
                                {
                                    addedPizzas &&
                                    addedPizzas.map(obj => (
                                        <CartItem type={obj.type} name={obj.name} size={obj.size} image={obj.imageUrl} totalPrice={items[obj.id].totalPrice}
                                        totalCount={items[obj.id].items.length} onClick={onRemoveItem} id={obj.id}/>
                                    ))
                                }
                            </div>
            ...
            Для того, чтобы заказать пиццу, перейди на главную страницу.
                                ...
                                <Link to="/">
                                    <div href="/" className="button button--black">
                                        <span>Вернуться назад</span>
                                    </div>
                                </Link>
                            </div>
